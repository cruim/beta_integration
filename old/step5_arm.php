	...
	function request_xml($shipping_docs, $shipping_orders, $shipping_order_rows, $lk, $pass) {
		...
		$oXMLout->writeAttribute("request_type", "101");
			...
			$oXMLout->writeAttribute("doc_txt", "");
				...
				$oXMLout->writeAttribute("delivery_type", "1"); //почта
				$oXMLout->writeAttribute("dev1direct_ctg", "2"); //международное отправление
				$oXMLout->writeAttribute("dev1country", "АРМЕНИЯ"); //международное отправление
				$oXMLout->writeAttribute("dev1mpo_mode", "2"); //авиа
				$oXMLout->writeAttribute("dev1mpo_kurs", str_replace('.', ',', "8.3336805700238") );   //с запятой!!!
