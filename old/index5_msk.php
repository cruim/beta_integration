	...
	function request_xml($shipping_docs, $shipping_orders, $shipping_order_rows, $lk, $pass) {
		...
		$oXMLout->writeAttribute("request_type", "101");
			...
			$oXMLout->writeAttribute("doc_txt", "");
				...
				$oXMLout->writeAttribute("delivery_type", "8");
				...
				$oXMLout->writeAttribute("clnt_phone",      $shipping_order['shipping_order_clnt_phone']);
                                #if (!is_null($shipping_order['shipping_order_delivery_date']))
				$oXMLout->writeAttribute("delivery_date",   $shipping_order['shipping_order_delivery_date']);
                                #if (!is_null($shipping_order['shipping_order_delivery_time']))
                $oXMLout->writeAttribute("delivery_time",      $shipping_order['shipping_order_delivery_time']);
                $oXMLout->writeAttribute("delivery_timeend",   $shipping_order['shipping_order_delivery_timeend']);
