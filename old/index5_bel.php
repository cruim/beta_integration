...
$xml = request_xml($shipping_docs, $shipping_orders, $shipping_order_rows);
...
curl_close($o_Curl);
	...
	function request_xml($shipping_docs, $shipping_orders, $shipping_order_rows) {
		...
		$oXMLout->writeAttribute("partner_id", "267"); //201  //275  //267  //280
		...
		$oXMLout->writeAttribute("request_type", "101");
			...
			$oXMLout->writeAttribute("doc_txt", "");
				...
				//$oXMLout->writeAttribute("dev1mail_type", "16");
				$oXMLout->writeAttribute("delivery_type", "1");
				$oXMLout->writeAttribute("dev1direct_ctg", "2"); //международное отправление
				$oXMLout->writeAttribute("dev1country", "БЕЛАРУСЬ"); //международное отправление
				$oXMLout->writeAttribute("dev1mpo_mode", "2"); //авиа
				$oXMLout->writeAttribute("dev1mpo_kurs", "1");
				...
				$oXMLout->writeAttribute("price", 		str_replace('.', ',', $shipping_order_row['shipping_order_row_price']) );
				$oXMLout->writeAttribute("clnt_price", 	str_replace('.', ',', $shipping_order_row['shipping_order_row_clnt_price']) );
